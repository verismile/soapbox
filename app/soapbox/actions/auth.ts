...
// Verismile: essa declaração corrige o erro do Typescript dizendo que a componente ReactNativeWebView não está declarada em window.
declare global {
...
export const verifyCredentials = (token: string, accountUrl?: string) => {
  ...
  return (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return baseClient(token, baseURL).get('/api/v1/accounts/verify_credentials').then(({ data: account }) => {
      ...
      if (account.avatar.endsWith('/avatars/original/missing.png')) dispatch({ type: 'ONBOARDING_START' }); // Verismile: ativar onboarding quando o avatar não estiver definido. TODO: melhorar
      return account;
    }).catch(error => {
...
export const logOut = () =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    // Verismile: verificar se está rodando dentro ao App, caso positivo enviar mensagem para React Native para excluir o local storage do app nativo.
    if (window.ReactNativeWebView) window.ReactNativeWebView.postMessage("logout");
