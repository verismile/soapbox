...
export const AccountRecord = ImmutableRecord({
  ...
  website: '',
  verified: true,
  full_name: '', // Verismile: adicionado full_name
...
// Trueface: adicionado a função normalizeFullname para incluir o full_name
const normalizeFullname = (account: ImmutableMap<string, any>) => {
  const fullname = account.get('full_name');
  return account.set('full_name', fullname);
};
...
export const normalizeAccount = (account: Record<string, any>) => {
  return AccountRecord(
    ImmutableMap(fromJS(account)).withMutations(account => {
      ...
      normalizeFullname(account); // Verismile
    }),
  );
};
